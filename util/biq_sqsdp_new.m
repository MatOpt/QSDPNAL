%%******************************************************
%% biq: generate SDP data for binary quadratic 
%%      programming
%%
%% X = [Y,y; y', 1];
%%******************************************************

   function [blk,A,C,b,B,d,SignP,fV,U,L,nonnegative] = biq_sqsdp_new(Q,options)
...
%% 
   ...
   At = At{1};
   A.At = At;
   A.Amap = @(X) diag(X) - [X(1:end-1,end);0];
   A.ATmap = @(y) ATmap(y);
%    n1 = n+1;   
%    Acell = cell(1,n); 
%    for k = 1:n
...
%    At = At{1};
%    A.At = At;
%    A.Amap = @(X) diag(X(1:end-1,1:end-1)) - X(1:end-1,end);
...
%%
   if (options == 3) 
      ...
      Bt = [Atmp1{1}, A1tmp1{1}, Atmp2{1}]; 
      d = [zeros(m2*2,1); -ones(m2,1);];
      B = Bt;
%       B.Bt = Bt;
%       B.Bmap = @(X) (mexsvec(blk,X)'*Bt)';
%       B.BTmap = @(y) {mexsmat(blk,Bt*y)};   
   end
end
