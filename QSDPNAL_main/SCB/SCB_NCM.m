%%*************************************************************************
%% QSDPNAL: Phase I
%% An inexact SCB based semi-Proximal ADMM for solving NCM problems:
...
%%*************************************************************************
   ...
   function [obj,Xi,S,yE,X,runhist,info] = SCB_NCM(blk,H,A,b,C,options,X,Xi,S,yE)
...
%% 
   ...
   if printyes
        fprintf('\n *******************************************************');
        fprintf('******************************************');
        fprintf('\n \t\t   SCB_NCM with  beginning gamma = %6.3f', gamma);
        fprintf('\n ******************************************************');
        fprintf('*******************************************\n');
        if printminoryes
            fprintf('\n n = %3.0f, mE = %3.0f',n, nE);
...
%  parameters for main solver
   ...
   % main file of SCB ADMM for solving NCM problem
   [X,yE,S,Xi,info_main,runhist] = ...
     SCB_NCMmain(Horg,Ainput,b,C,parmain,X,yE,S,Xi);
...
%%-----------------------------------------------------------------
   ...
   dualfeasorg = max(norm(HXi+S+AtyE-Corg,'fro')/normCorg);
