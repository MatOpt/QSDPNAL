...
%%*************************************************************************
   ...
   function [obj,Z,W,QW,S,yE,yI,X,runhist,info] = qsdpnal_BIQ(blk,Q,A,b,C,B,d,options)
%% General options
   rng('default');
   maxiter = 5000;
   stoptol = 1e-6;
...
%% generate initial point through QSDPNAL-Phase I (SCB_qsdp)
   ...
   sGS.op.maxiter = 1500; % maxiter of initial stage
...
%%-----------------------------------------------------------------
   ...
   dualobj = full(borg'*yE + sum(sum(Z.*Ztmp)) - 0.5*sum(sum(W.*QW)) + dTyI);   
   obj = [primobj,dualobj];
   if ~isempty(L)
